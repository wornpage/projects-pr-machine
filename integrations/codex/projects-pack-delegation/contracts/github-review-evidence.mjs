...
async function collectReviewState(expected, rest, graphql, resource, authorId) {
  ...
  requireThat(complete, 'incomplete_review_observation');
  return { approvals, evidenceSha256: dataDigest({ expected, authorId,
    reviews: observed.sort((a, b) => a.id - b.id), threads: threads.sort() }) };
}
 ...
 * Two complete mutable-state observations must agree, bracketed by identity
 * reads. This is NOT an atomic snapshot or durable authorization: GitHub can
 * still change after any read. Enforce current policy again at the actual effect.
 */
export async function readGitHubReviewEvidence(expectedInput, clientInput) {
  try {
    ...
    const first = await collectReviewState(expected, rest, graphql, resource, authorId);
    requireThat(identity(await rest(resource), expected) === authorId, 'review_revision_mismatch');
    // Identity can stay unchanged while an approval is dismissed or a thread is
    // reopened. Repeat ALL mutable-state pages, not just the PR identity read.
    // Drift is a refusal, never an automatic retry following new authorization.
    const second = await collectReviewState(expected, rest, graphql, resource, authorId);
    requireThat(identity(await rest(resource), expected) === authorId, 'review_revision_mismatch');
    requireThat(first.evidenceSha256 === second.evidenceSha256, 'review_state_changed');
    return captureData({ schemaVersion: 1, kind: 'github-review-evidence', status: 'validated',
      ...
      reviewerIds: second.approvals.map(review => review.actorId), reviewIds: second.approvals.map(review => review.id),
      evidenceSha256: second.evidenceSha256 });
  } catch (error) {
